 ...
 */
(function(c,d){var b="3.0.2";
function g(t){if(c.fn.cycle.debug){f(t);}}function f(){if(window.console&&console.log){console.log("[cycle] "+Array.prototype.join.call(arguments," "));
...
w(false,t,s);k(s);return false;case"prev":case"next":x=c(s).data("cycle.opts");if(!x){f('options not found, "prev/next" ignored');return false;}if(typeof t=="string"){x.oneTimeFx=t;
}c.fn.cycle[v](x);return false;default:v={fx:v};}return v;}else{if(v.constructor==Number){var u=v;v=c(s).data("cycle.opts");if(!v){f("options not found, can not advance slide");
...
w.css({"margin-left":H.width?((H.width-w.width())/2)+"px":0,"margin-top":H.height?((H.height-w.height())/2)+"px":0});});}var I=(H.containerResize||H.containerResizeHeight)&&C.innerHeight()<1;
...
}k(u,true);});}if(o(H)===false){return false;}var v=false;y.requeueAttempts=y.requeueAttempts||0;Q.each(function(){var w=c(this);this.cycleH=(H.fit&&H.height)?H.height:(w.height()||this.offsetHeight||this.height||w.attr("height")||0);
this.cycleW=(H.fit&&H.width)?H.width:(w.width()||this.offsetWidth||this.width||w.attr("width")||0);if(w.is("img")){var S=(this.cycleH===0&&this.cycleW===0&&!this.complete);
if(S){if(J.s&&H.requeueOnImageNotLoaded&&++y.requeueAttempts<100){f(y.requeueAttempts," - img slide not loaded, requeuing slideshow: ",this.src,this.cycleW,this.cycleH);
