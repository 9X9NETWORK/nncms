...
(function(nn) {
	...
	nn.api = function(method, resourceURI, parameter, callback, dataType) {
        ...
        // fake for src
        var fakeKey = 'fakeapi/json/',
            fakeIdx = resourceURI.indexOf(fakeKey),
            fakePos = 0,
            fakePreURI = '',
            fakePostURI = '';
        if (fakeIdx >= 0) {
            fakePos = fakeIdx + fakeKey.length;
            fakePreURI = resourceURI.substr(0, fakePos);
            fakePostURI = resourceURI.substr(fakePos);
